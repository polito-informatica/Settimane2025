"""
Scrivere un programma Python che legga un file "dati_sensori.csv" contenente dati ambientali (es. temperatura e umidità) in ordine temporale di acquisizione in formato CSV e che supporti le seguenti funzionalità:

- PREPROCESSING DEI DATI: durante la lettura, gestire le eccezioni principali (FileNotFoundError, IOError, ValueError), pre-processando i dati e scartando le righe con valori mancanti,
 celle vuote, colonne in eccesso/difetto o valori non numerici. Creare il file "qualità_dati.csv" che indichi quanti record validi e quanti scartati sono stati letti. Restituire la tabella contenente solo dati validi.

- RIORGANIZZAZIONE: organizzare i dati validi raggruppando le misure per parametro in file separati (ad esempio "dati_temperatura.csv"   e "dati_umidità.csv", se presenti nel file originale).
